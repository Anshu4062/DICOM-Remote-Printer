    ...
    if (ext === '.zip' || file.type === 'application/zip') {
      ...
      for (const entry of entries) {
        ...
        if (isDicomBuffer(entryBuf)) {
          ...
          const outName = `${Date.now()}_${safeName.endsWith('.dcm') ? safeName : safeName + '.dcm'}`;
      ...
      if (savedFiles.length === 0) {
        return NextResponse.json({ error: 'No DICOM files found in ZIP' }, { status: 400 });
      }
      return NextResponse.json({ message: 'ZIP processed', files: savedFiles }, { status: 200 });
    }

    // Case 2: Raw DICOM with or without extension
    if (!isDicomBuffer(buffer)) {
      return NextResponse.json({ error: 'Uploaded file is not a valid DICOM' }, { status: 400 });
    }

    // Generate unique filename
    const timestamp = Date.now();
    const baseName = file.name || 'dicom';
    const filename = `${timestamp}_${baseName.endsWith('.dcm') ? baseName : baseName + '.dcm'}`;
